...
def main():
    ...
    N = 80
    dt = 0.5
    x0 = np.array([4.12, 4.94, 0.0, 0.0])  # [x, y, xdot, ydot]
    ...
    regions = [
        ...
        {'xl': 1.5,  'xu': 1.5,  'yl': 1.6, 'yu': 1.6},  
        {'xl': 4.8,  'xu': 4.8,  'yl': 0.8, 'yu': 0.8},  
        {'xl': 4.5,  'xu': 4.5,  'yl': 2.0, 'yu': 2.0},  
        {'xl': 2.2,  'xu': 2.2,  'yl': 4.4, 'yu': 4.4}, 
        {'xl': 7.8,  'xu': 7.8,  'yl': 0.2, 'yu': 0.2},  
        {'xl': 9.5,  'xu': 9.5,  'yl': 2.0, 'yu': 2.0},  
        {'xl': 10.0, 'xu': 11.0, 'yl': 0.0, 'yu': 5.2}, 
        {'xl': 6.0,  'xu': 10.0, 'yl': 0.0, 'yu': 1.0}, 
        {'xl': 7.5,  'xu': 8.5,  'yl': 1.0, 'yu': 1.4},   
        {'xl': 6.0,  'xu': 10.0, 'yl': 1.4, 'yu': 2.4},  
        {'xl': 6.0,  'xu': 10.0, 'yl': 2.8, 'yu': 3.8},  
        {'xl': 6.0,  'xu': 10.0, 'yl': 4.2, 'yu': 5.2},  
        {'xl': 7.5,  'xu': 8.5,  'yl': 3.8, 'yu': 4.2}
    ]
    
    # Region costs from config
    scaling = 50
    region_costs = scaling * np.random.uniform(0, 1, 21)
    
    # Regions of interest (targets to visit)
    region_of_interest = [8, 9, 10, 11, 12, 13]
    ...
    # Build STL specification for 6 targets: [8, 9, 10, 11, 12, 13]
    # Sequential constraints: visit 9 before 8, visit 11 before 10, visit 13 before 12
    ...
    not_visit12 = ConvexSetPredicate(name='nf', edge_no=region_of_interest[4], neg=True)
    ...
    visit13 = ConvexSetPredicate(name='nf', edge_no=region_of_interest[5], neg=False)

    not_visit8_until_visit9 = not_visit8.until(visit9, 0, params.N)
    not_visit10_until_visit11 = not_visit10.until(visit11, 0, params.N)
    not_visit12_until_visit13 = not_visit12.until(visit13, 0, params.N)
    ...
    visit12_eventually = ConvexSetPredicate(name='nf', edge_no=region_of_interest[4], neg=False).always(0, 3).eventually(0, params.N-5)
    visit13_eventually = ConvexSetPredicate(name='nf', edge_no=region_of_interest[5], neg=False).always(0, 3).eventually(0, params.N-5)

    # Combined specification
    specification = not_visit8_until_visit9 & not_visit10_until_visit11 & not_visit12_until_visit13 & \
                    (visit8_eventually | visit10_eventually | visit12_eventually) & \
                    visit9_eventually & visit11_eventually & visit13_eventually
